...
class Shares extends BasicAdmin
{
    ...
    public function index(){
        ...
        $shares_list=Db::name($this->shares_table)
                    ...
                    ->field('shares.*,
                        ...
                        user.last_name_py,user.first_name_py,user.address_py,
    ...
    public function out(){
        ...
        ->setCellValue('D1', '代理公司')
        ->setCellValue('E1', '姓')
        ->setCellValue('F1', '名')
        ->setCellValue('G1', '姓-拼音')
        ->setCellValue('H1', '名-拼音')
        ->setCellValue('I1', '股票')
        ->setCellValue('J1', '金额')
        ->setCellValue('K1', '手机')
        ->setCellValue('L1', '身份证号')
        ->setCellValue('M1', '地址')
        ->setCellValue('N1', '地址-拼音')
        ->setCellValue('O1', '银行信息')
        ->setCellValue('P1', '银行账户')
        ->setCellValue('Q1', '时间')
        ->setCellValue('R1', '银行流水信息')
        ->setCellValue('S1', '记录信息');
        ...
        for ($i = 2; $i <= $count+1; $i++) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $sql[$i-2]['last_name']);
            $objPHPExcel->getActiveSheet()->setCellValue('F' . $i, $sql[$i-2]['first_name']);
            $objPHPExcel->getActiveSheet()->setCellValue('G' . $i, $sql[$i-2]['last_name_py']);
            $objPHPExcel->getActiveSheet()->setCellValue('H' . $i, $sql[$i-2]['first_name_py']);
            $objPHPExcel->getActiveSheet()->setCellValue('I' . $i, $sql[$i-2]['shares']);
            $objPHPExcel->getActiveSheet()->setCellValue('J' . $i, $sql[$i-2]['money']);
            $objPHPExcel->getActiveSheet()->setCellValue('K' . $i, "'".$sql[$i-2]['phone']);
            $objPHPExcel->getActiveSheet()->setCellValue('L' . $i, "'".$sql[$i-2]['code']);
            $objPHPExcel->getActiveSheet()->setCellValue('M' . $i, $sql[$i-2]['address']);
            $objPHPExcel->getActiveSheet()->setCellValue('N' . $i, $sql[$i-2]['address_py']);
            $objPHPExcel->getActiveSheet()->setCellValue('O' . $i, $sql[$i-2]['bankinfo']);
            $objPHPExcel->getActiveSheet()->setCellValue('P' . $i, "'".$sql[$i-2]['banknum']);
            $objPHPExcel->getActiveSheet()->setCellValue('Q' . $i, $sql[$i-2]['update_time']);
            $objPHPExcel->getActiveSheet()->setCellValue('R' . $i, $sql[$i-2]['remark']);
            $objPHPExcel->getActiveSheet()->setCellValue('S' . $i, $sql[$i-2]['infomark']);
        }
